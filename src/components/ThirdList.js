...
const ThirdList = ({ title }) => {
  const [todos, setTodos] = useState([]);
  const [input, setInput] = useState('');

  useEffect(() => {
    db.collection('monthly todos')
  ...
  const addTodo = event => {
    ...
    const todo = db.collection('monthly todos').add({
      todo: input,
  ...
  const copyURL = () => {
    ...
    alert('Monthly todos url copied to clipboard!');
  };
